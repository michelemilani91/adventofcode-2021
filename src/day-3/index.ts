...
const countBitsPerColumn = (a: readonly BinaryNumber[]): readonly BinaryCounter[] =>
    a.reduce<BinaryCounter[]>((counter, value) =>
...
const calculateGammaRate = (a: readonly BinaryCounter[]): number => Number.parseInt(a.map(x => x.numberOf0 > x.numberOf1 ? '0' : '1').join(''), 2);

const calculateEpsilonRate = (a: readonly BinaryCounter[]): number => Number.parseInt(a.map(x => x.numberOf0 < x.numberOf1 ? '0' : '1').join(''), 2);
...
const calculateRating = (array: readonly BinaryNumber[], evaluatedValue: '0' | '1', column: number = 0): BinaryNumber => {
    const { numberOf1, numberOf0 } = countBitsPerColumn(array)[column];
    const bitValue = evaluatedValue == '1' ? numberOf1 >= numberOf0 : numberOf1 < numberOf0;
    const filteredArray = array.filter(a => Boolean(Number(a[column])) === bitValue);
    return filteredArray.length === 1 ? filteredArray[0] : calculateRating(filteredArray, evaluatedValue, column + 1);
};
...
export const partTwo = flow(
    ...
    TE.map((a: readonly string[][]) => a.map(parseBinaryNumber)),
    TE.chainEitherKW(E.sequenceArray),
    TE.map((a: readonly BinaryNumber[]) => calculateOxygenGeneratorRating(a) * calculateCO2ScrubberRating(a))
);


partTwo('src/day-3/input.txt')()
    .then(a => E.isRight(a) ? console.log('Part two: What is the life support rating of the submarine? ', a.right) : console.log('Error: ', a.left))
    .catch(a => console.log('Error: ', a));
